...
def parse_line(ln):
    rng, letter, pswd = ln.split(" ")
    min, max = [int(n) for n in rng.split("-")]
    letter = letter[:-1]
...
for (start, end), letter, passwd in lines:
    if passwd.count(letter) in range(start, end + 1):
        valid += 1
...
valid = 0
for (start, end), letter, passwd in lines:
    elems = passwd[start - 1], passwd[end - 1]
    if letter in elems and elems[0] != elems[1]:
        valid += 1
print(valid)
